...
int main(int argc, char * argv[])
{
    ...
    SDL_Event event;
	running = true;
	while(av_read_frame(screenCtx, screenPacket) >= 0 && running)
	{
		...
		if(screenPacket->stream_index == screenVideoStream)
		{
			...
			while(screenRet >= 0){
				...
				if(++i >= 0){
					...
					if(keyboard[SDL_SCANCODE_F2] ){
						saveFrame(pFrameRGB, screenCodecCtx->width, screenCodecCtx->height, i);
					}
				}
			}
